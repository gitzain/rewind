...
public class SettingsWindow : Gtk.Dialog{
	...
	private void cell_sched_enable_toggled (string path){
		TreeIter iter;
		Gtk.ListStore model = (Gtk.ListStore)tv_schedule.model;
		bool enabled;
		int count;
	...
	private void cell_remove_limit_edited (string path, string new_text) {
		...
		TreeIter iter;
		Gtk.ListStore model = (Gtk.ListStore)tv_remove.model;
		model.get_iter_from_string (out iter, path);
	...
	private void refresh_tv_schedule(){
		...
		Gtk.ListStore model = new Gtk.ListStore(3, typeof(bool), typeof(string), typeof(string));
	...
	private void refresh_tv_remove(){
		...
		Gtk.ListStore model = new Gtk.ListStore(3, typeof(string), typeof(int), typeof(string));
	...
	private void switch_schedule_changed(){
		...
		if (switch_schedule.active){
			...
			if(!atleast_one_enabled){
				TreeIter iter;
				Gtk.ListStore store = (Gtk.ListStore) tv_schedule.model;
				bool enabled; 
				int index = -1;
